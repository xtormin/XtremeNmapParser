...
(function () {
  ...
  function preferredLang() {
    ...
    } catch (err) { /* private mode: fall through to the browser */ }
  ...
  var TARGET_SHAPES = [
    ...
    { id: "nmap", build: function (rows) {
        var ports = unique(rows.map(function (r) { return r.num; })).sort(function (a, b) { return a - b; });
        var hosts = unique(rows.map(function (r) { return r.ip; }));
        return ["nmap -sV -sC -p " + ports.join(",") + " " + hosts.join(" ")]; } }
  ];
  ...
  function init() {
    ...
    el("groups").addEventListener("click", function (event) {
      ...
      if (shape) {
        ...
        var wanted = TARGET_SHAPES.filter(function (t) {
          return t.id === shape.getAttribute("data-shape");
        })[0];
